...
const addInventoryItem = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      console.log(`🔄 Syncing product to ${branches.length} branches...`);
      ...
      for (const branch of branches) {
        try {
          ...
          if (existingBranchProduct) {
            ...
            existingBranchProduct.quantity += newItem.quantity;
            await existingBranchProduct.save();
            console.log(`✅ Updated existing product in branch ${branch.name}: ${newItem.name}`);
            syncResults.push({ branch: branch.name, status: 'updated' });
          } else {
            ...
            console.log(`✅ Added new product to branch ${branch.name}: ${newItem.name}`);
            syncResults.push({ branch: branch.name, status: 'added' });
          }
        } catch (branchError) {
          console.error(`❌ Error syncing to branch ${branch.name}:`, branchError);
          syncResults.push({ branch: branch.name, status: 'error', error: branchError.message });
        }
      }
      
      console.log(`🎉 Product sync completed. Results:`, syncResults);
      
      return res.status(201).json({ 
        success: true,
        message: "Product added to factory inventory and synced to all branches successfully",
        item: newItem,
        syncResults: syncResults
      });
    } catch (syncError) {
      console.error('❌ Error syncing to branches:', syncError);
      // Still return success for factory inventory, but note sync issue
      return res.status(201).json({ 
        success: true,
        message: "Product added to factory inventory, but there was an issue syncing to some branches",
        item: newItem,
...
const addProductAndSyncToBranch = async (req, res, next) => {
  ...
  try {
    ...
    // Auto-sync to all branches (not just the specified one)
    let syncResults = [];
    
    try {
      // Get all active branches
      const branches = await Branch.find({ status: 'Active' });
      console.log(`🔄 Auto-syncing product to ${branches.length} branches...`);
      ...
      console.error('❌ Error syncing to branches:', syncError);
      syncResults.push({ error: 'Failed to sync to branches', details: syncError.message });
    }
...
// POST: Sync all factory products to all branches (utility function)
const syncAllProductsToBranches = async (req, res, next) => {
  try {
    const BranchInventory = require("../Model/BranchInventoryModel");
    const Branch = require("../Model/BranchModel");
    
    // Get all factory products
    const factoryProducts = await Inventory.find();
    console.log(`🔄 Found ${factoryProducts.length} factory products to sync`);
      ...
      for (const branch of branches) {
        try {
          ...
          if (!existingBranchProduct) {
            ...
            await branchProduct.save();
            console.log(`✅ Synced ${product.name} to ${branch.name}`);
            syncResults.push({ 
            ...
            console.log(`⏭️ Product ${product.name} already exists in ${branch.name}`);
            syncResults.push({ 
              product: product.name, 
              branch: branch.name, 
          ...
          console.error(`❌ Error syncing ${product.name} to ${branch.name}:`, branchError);
          syncResults.push({ 
            product: product.name, 
            branch: branch.name, 
            status: 'error', 
    ...
    console.log(`🎉 Sync all products completed. Results:`, syncResults);
    
    return res.status(200).json({
      success: true,
      message: "All factory products synced to branches successfully",
      syncResults: syncResults,
      summary: {
        totalProducts: factoryProducts.length,
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to sync all products to branches",
      error: err.message
    });
  }
};
...
exports.syncAllProductsToBranches = syncAllProductsToBranches;
