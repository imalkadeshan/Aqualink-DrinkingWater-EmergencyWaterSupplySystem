...
function BranchInventoryPage({ branchId, branchName, showNotification }) {
  ...
  const fetchAvailableProducts = async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/FactoryRequests/available-products', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      
      if (response.data.success) {
        // Update categories with available products from factory
        const productNames = response.data.products.map(product => product.name);
        setCategories(prev => {
          const combined = [...new Set([...prev, ...productNames])];
