...
const DriverDashboard = () => {
  ...
  const [processedBonusDeliveries, setProcessedBonusDeliveries] = useState(new Set());
  const [isCheckingBonuses, setIsCheckingBonuses] = useState(false);
  ...
  useEffect(() => {
    if (user?._id) {
      ...
      // Add a small delay before checking bonuses to prevent race conditions
  ...
  const addBonusNotification = (delivery) => {
    ...
    const notificationKey = `${delivery.type}-${brigadeName}-${bonusAmount}`;
    
    // Check if a similar notification already exists
    setBonusNotifications(prev => {
      const existingNotification = prev.find(n => 
        n.delivery?.type === delivery.type && 
        n.delivery?.emergencyDetails?.brigadeName === brigadeName &&
        n.amount === bonusAmount
      );
      
      if (existingNotification) {
        console.log('⚠️ Duplicate bonus notification prevented:', notificationKey);
      ...
      return [notification, ...prev.slice(0, 4)]; // Keep only latest 5
    });
    
    // Auto-remove notification after 10 seconds
    setTimeout(() => {
      setBonusNotifications(prev => prev.filter(n => n.key !== notificationKey));
    }, 10000);
  };
  ...
  const checkEmergencyDeliveriesForBonuses = async () => {
    ...
    if (isCheckingBonuses) {
      console.log('⚠️ Bonus check already in progress, skipping...');
      return;
    }
    
    try {
      setIsCheckingBonuses(true);
      ...
      if (user?._id) {
        ...
        if (response.ok) {
          ...
          const deliveriesWithoutBonuses = completedEmergencyDeliveries.filter(delivery => 
            ...
            !processedBonusDeliveries.has(delivery._id) // Prevent processing same delivery multiple times
          );
          
          if (deliveriesWithoutBonuses.length > 0) {
            console.log(`🎯 Found ${deliveriesWithoutBonuses.length} deliveries needing bonus processing`);
            
            // Mark these deliveries as being processed
            const newProcessedIds = new Set(processedBonusDeliveries);
            deliveriesWithoutBonuses.forEach(delivery => {
              newProcessedIds.add(delivery._id);
            });
            setProcessedBonusDeliveries(newProcessedIds);
            
            // Create bonuses for these deliveries
            await createMissingBonuses(deliveriesWithoutBonuses);
          } else {
            console.log('✅ No new deliveries need bonus processing');
          }
        }
      }
    } catch (error) {
                ...
                <button
                  onClick={async () => {
                    try {
                      ...
                      // Only check bonuses if not already checking
                      if (!isCheckingBonuses) {
